...
const EditJobSeekerForm = ({ jobSeeker }: EditJobSeekerFormProps) => {
  ...
  const [newLanguage, setNewLanguage] = useState("");
  const router = useRouter();
  const t = useTranslations("EditJobSeekerForm");
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t("form.portfolio")}</FormLabel>
                  <FormControl>
                    <Input type="url" {...field} />
                  </FormControl>
                  <FormMessage />
                ...
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      {Object.entries(Availability).map(([key, value]) => (
                        <SelectItem key={key} value={value}>
                          {t(`availability.${value.toLowerCase()}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t("form.preferredJobTypes")}</FormLabel>
                  <div className="grid grid-cols-2 gap-4">
                    {Object.entries(JobType).map(([value]) => (
                      <div key={value} className="flex items-center space-x-2">
                        <Checkbox
                          id={value}
                          checked={field.value?.includes(value)}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t("form.expectedSalary")}</FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                      {...field}
